...
export default class Order extends React.Component {
 ...
 this.setState({
  ...
  const strCall ='https://webrestservice.seral-service.com/AppTPV/Orders/001/'+ GLOBAL.idUsuario + '/'+ GLOBAL.idCliente + 
                 '/'+ GLOBAL.idCocina + '/' + GLOBAL.DiaSel + '/' + GLOBAL.MesSel +'/' + GLOBAL.AnioSel +'/' + GLOBAL.MenuSel + '/' + GLOBAL.IngestaSel
  console.log(strCall)
   ...
   return (
     ...
     <View style={styles.container2}>
       <ImageBackground source={background} style={styles.background} resizeMode="cover" blurRadius={2}>
        <Text style={{marginTop:20, textAlign:'center',  color: "#FFF", fontSize: 18}}>
            {this.state.title1}
        </Text>
          ...
          renderItem={({item}) => 
            ...
            <View style={styles.listItem}>
                ...
                <View style={{alignItems:"center",flex:1}}>
                  <Text  onPress={this.GetItem.bind(this, item.Receta)} style={{fontWeight:"bold"}}>{item.Categoria}</Text>
                  <Text style={{height:35}}>{item.Receta}</Text>
                </View>
