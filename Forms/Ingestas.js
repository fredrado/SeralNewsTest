...
export default class Ingestas extends React.Component {
 ...
 constructor(props) {
  ...
  //Muestro detalle
 ShowDetail(ingesta, nombremenu){
    //Parametros
    GLOBAL.IngestaSel=ingesta
    
    //Abro pantalla
    this.props.navigation.push('Tabs', {itemId:'1111'}) }
...
console.log(GLOBAL.idCliente)
console.log(GLOBAL.idCocina)
console.log(GLOBAL.MenuSel)
console.log(GLOBAL.DiaSel)
console.log(GLOBAL.MesSel)
console.log(GLOBAL.AnioSel)
  //Llamada a WebService
  const strCall ='https://webrestservice.seral-service.com/AppMenus/Ingestas/001/'  + GLOBAL.idCliente + '/' + GLOBAL.idCocina + '/' +
                  GLOBAL.MenuSel + '/' + GLOBAL.DiaSel + '/' + GLOBAL.MesSel +'/' + GLOBAL.AnioSel 
     ...
     <View style={styles.container2}>
       <ImageBackground source={background} style={styles.background} resizeMode="cover" blurRadius={2}>
        ...
        <FlatList style={{flex:1}}
          ...
          renderItem={({item}) => 
            <TouchableOpacity activeOpacity={.5} onPress={this.ShowDetail.bind(this, item.Periodo,  this.GetItem(item.Periodo))}>
            <View style={styles.listItem}>
 
                <View style={{alignItems:"center",flex:1}}>
                  
                  <Text onPress={this.ShowDetail.bind(this, item.Periodo,  this.GetItem(item.Periodo))}  
                                style={{height:35}}> {this.GetItem(item.Periodo)}</Text>
                </View>
